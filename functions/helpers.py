...
def get_obj_colliders(obj, vertex_group_name, col_export):
    if obj.type != 'MESH':
        raise ValueError("Object must be a mesh")
    ...
    new_obj.name = f"{src_name}_Col_{vertex_group_name}"
    
    bpy.ops.object.select_all(action='DESELECT')
    return

def get_obj_mesh(obj, vertex_group_name, col_export):
    if obj.type != 'MESH':
        raise ValueError("Object must be a mesh")
